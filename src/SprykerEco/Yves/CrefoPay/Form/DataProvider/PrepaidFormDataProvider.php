...
class PrepaidFormDataProvider implements StepEngineFormDataProviderInterface
{
    /**
    ...
    {
        return [
            PrepaidSubForm::CREFO_PAY_SHOP_PUBLIC_KEY => $this->config->getPublicKey(),
            PrepaidSubForm::CREFO_PAY_ORDER_ID => $quoteTransfer->getCrefoPayTransaction()->getCrefoPayOrderId(),
            PrepaidSubForm::CREFO_PAY_SECURE_FIELDS_API_ENDPOINT => $this->config->getSecureFieldsApiEndpoint(),
            PrepaidSubForm::CREFO_PAY_SECURE_FIELDS_PLACEHOLDERS => $this->config->getSecureFieldsPlaceholders(),
        ];
    }
}

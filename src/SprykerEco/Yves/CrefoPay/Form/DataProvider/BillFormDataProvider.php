...
class BillFormDataProvider implements StepEngineFormDataProviderInterface
{
    ...
    {
        return [
            BillSubForm::CREFO_PAY_SHOP_PUBLIC_KEY => $this->config->getPublicKey(),
            BillSubForm::CREFO_PAY_ORDER_ID => $quoteTransfer->getCrefoPayTransaction()->getCrefoPayOrderId(),
            BillSubForm::CREFO_PAY_SECURE_FIELDS_API_ENDPOINT => $this->config->getSecureFieldsApiEndpoint(),
            BillSubForm::CREFO_PAY_SECURE_FIELDS_PLACEHOLDERS => $this->config->getSecureFieldsPlaceholders(),
        ];
    }
}
